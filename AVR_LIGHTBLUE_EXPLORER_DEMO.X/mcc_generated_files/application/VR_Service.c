...
const iVR_FunctionPtrs_t VR_BRIDGE = {
    .bridgeTx = USART1_Write,
    .bridgeRx = USART1_Read,
};
...
void VR_AllData(bool connectedState)
{
    ...
    *payloadButton = '\0';
    ...
    led = (0x07 & ( ((connectedState & 0x01) << 2) + (VR_GetErrorLedValue() << 1) + (VR_GetDataLedValue()) ) );
    *payloadLed = '\0';
    
    gpio = (0x03 & (((GpioInput_2() & 0x40) >> 5) + ((GpioInput_1() & 0x02) >> 1) ) );
    *payloadGpio = '\0';
...
void VR_LedState(void)
{
    ...
    led = (0x03 & ((VR_GetErrorLedValue() << 1) + VR_GetDataLedValue()));
    *payload = '\0';
...
{
    if (strcmp(serialData, "data off") == 0)
    {
        DataLedOff();
    }
    else if (strcmp(serialData, "data on") == 0)
    {
        DataLedOn();
    }
    else if (strcmp(serialData, "err off") == 0)
    {
        ErrorLedOff();
    }
    else if (strcmp(serialData, "err on") == 0)
    {
        ErrorLedOn();
    }
    else
...
bool VR_InData(char* serialData)
{
    if (strcmp(serialData, "1") == 0)
    {
        DataLedOff();
    }
    else if (strcmp(serialData, "2") == 0)
    {
        DataLedOn();
    }
    else if (strcmp(serialData, "3") == 0)
    {
        ErrorLedOff();
    }
    else if (strcmp(serialData, "4") == 0)
    {
        ErrorLedOn();
    }
    else
